...
void readFifo(char*);
void printFifo(char*);
void manchDecode(char*);
void printManch(char*);
int preDecryptionChk(char*, FILE*, FILE*);
// FIFO test
					...
					if (rbuf[i*2+2] == 'x'){
						...
						for (j = i + 1; j < size; ++j)
							if (rbuf[j*2+2] == 'x')
								...
								fprintf(fpLog, " %02x", rbuf[j*2+1]);
						break;
					}
					rbuf[i] = decrypt(rbuf[i*2+1]);
					char val = rbuf[i];
...
void manchDecode(char *buf){
	unsigned char head, val, val2, temp, i;
	for (head=1; head<64; head += 2){
		val = buf[head];
		val2 = buf[head + 1];
		temp = 0;
		for (i=0; i<4; ++i){
			temp <<= 1;
			if (((val&0x80)^(val&0x40)) == 0){
				buf[head+1] = 'x';
				break;
			} else
				//temp |= (val&0x40) >> 2;
				temp |= (val&0x80) >> 3;
			if (((val2&0x80)^(val2&0x40)) == 0){
				buf[head+1] = 'x';
				break;
			} else
				//temp |= (val2&0x40) >> 6;
...
void printManch(char *buf){
	int i;
	printf("\n");
	for (i=1; i<=32; ++i)
		if (buf[i*2] == 'x')
			printf("Manch[%d]='x'%c", i, i%5==0?'\n':'\t');
		else
			printf("Manch[%d]=%02x%c", i, (int)buf[i*2-1], i%5==0?'\n':'\t');
}	

int preDecryptionChk(char *buf, FILE *fpLog, FILE *fpData){
	printf("\nproduct_ID=%02x, msg_len=%d, PIP=%04x\n", buf[1], buf[3], (uint16_t)(buf[5]<<8)|buf[7]);
	int retVal = 0;
	if (buf[2] == 'x'){
		fprintf(fpData, " FifoSize %#02x Product_ID_corrupted\n", buf[0]);
		retVal = -1;
	} else
	if (buf[4] == 'x' || buf[3] > 32){
		fprintf(fpData, " FifoSize %#02x Payload_size_corrupted\n", buf[0]);
		retVal = -2;
	} else
	if (buf[6] == 'x' || buf[8] == 'x'){
		fprintf(fpData, " FifoSize %#02x Encryption_PIP_corrupted\n", buf[0]);
		retVal = -3;
	}
	if (retVal < 0){
		int i;
		for (i = 0; i < 28; ++i)
			if (buf[i*2+2] == 'x')
				...
				fprintf(fpLog, " %02x", buf[i*2+1]);
		fprintf(fpLog, "\n");
	} else
		fprintf(fpLog, " %02x %d %02x %02x", buf[1], buf[3], buf[5], buf[7]);
