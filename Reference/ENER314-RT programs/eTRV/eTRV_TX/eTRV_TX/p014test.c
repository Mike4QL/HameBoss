...
// receive in variable length packet mode and display. Data with swapped first 2 bytes
uint32_t msgCnt = 0, msgLost = 0, lastId = 0;
...
int main(int argc, char **argv){
	...
	while(1){
		...
		while(regRead(ADDR_IRQFLAGS2) & MASK_FIFONOTEMPTY)
		{
		...
		while (msg.state != S_FINISH)
		{
			...
			if (msg.bytesToRead == 0 || msg.msgSize == 0){
				...
				printf("%02d:%02d:%02d %d ERROR: bytes to read=%d, msgSize=%d\n", tmp->tm_hour, tmp->tm_min, tmp->tm_sec, msgCnt, msg.bytesToRead, msg.msgSize);
			...
			{
				msg.buf[msg.bufCnt] = decrypt(msg.buf[msg.bufCnt]);
			}
			msg.value = (msg.value << 8) | msg.buf[msg.bufCnt];
			++msg.bufCnt;
			--msg.bytesToRead;
			--msg.msgSize;
...
static regSet_t regSetup[] = {
	...
	{0x37, 0xA2},								// CRC on -> CRC off, manchester
 ...
 void msgNextState(msg_t *msg){				// switch and initialize next state
	...
	switch (msg->state)
	{
		...
		case S_CRC:
			...
			{
				uint16_t value = (msg->buf[10] << 8) + msg->buf[11];
				if (msg->buf[8] != 0xAA){
					int i;
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							currentTime = time(NULL);
							tmp = gmtime(&currentTime);
							msgLost += (value - msgNr - 1) % 65536;
							printf("%02d:%02d:%02d %d/%d (%.2f\%) MSGs LOST: +%d    MSGnr: %d - %d\n", tmp->tm_hour, tmp->tm_min, tmp->tm_sec, msgCnt+msgLost, msgCnt, (double)msgCnt/(msgCnt+msgLost)*100, (value - msgNr -1) % 65536, (msgNr +1) % 65536, (value - 1) % 65536);
			...
			else
			{
				...
				currentTime = time(NULL);
				tmp = gmtime(&currentTime);
				printf("%02d:%02d:%02d %d CRC FAIL\n", tmp->tm_hour, tmp->tm_min, tmp->tm_sec, msgCnt);
			}
			break;
		case S_FINISH:
			...
			if (msg->msgSize > 0){
				time_t currentTime;
						...
						currentTime = time(NULL);
						tmp = gmtime(&currentTime);
				printf("%02d:%02d:%02d %d Shouldn't be there more data?!\n", tmp->tm_hour, tmp->tm_min, tmp->tm_sec, msgCnt);
			}
			msg->msgSize = 1;
