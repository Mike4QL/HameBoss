...
int main(int argc, char **argv){
	...
	while(1){
		if (regRead(0x38) != 0x1d){
			printf("\nreg 0x38 value has been changed to %02x. Reload val", regRead(0x38));
			regWrite(0x38, 0x1d);
			delay(10000);
		}
		if (regRead(0x3C) != 0x1c){
			printf("\nreg 0x3C value has been changed to %02x, Reaload val", regRead(0x3C));
			regWrite(0x38, 0x1C);
		}
