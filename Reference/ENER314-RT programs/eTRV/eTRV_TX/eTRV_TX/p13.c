...
static uint8_t buf[0x02], ledState = 0;
...
// receive in variable length packet mode, display and resend. Data with swapped first 2 bytes, testing purposes
...
void sendMessage(msg_t *msg){
	...
	msg->buf[0] += 3;
	size = msg->buf[0];
	msg->buf[size -6] = 0x73;			// Switch state
	msg->buf[size -5] = 0;
	msg->buf[size -4] = ledState;
	msg->buf[size -3] = 0;				// CRC type
	msg->buf[6] = 0x77;					// Sensor ID
	msg->buf[1] = 0x01;					// Product ID
	setupCrc(msg->buf);
	if (ledState == 13){				// malicious message
		msg->buf[0] = 232;
		msg->buf[size -6] = 0xff;
		msg->buf[6] = 0x13;					// Sensor ID
	}
